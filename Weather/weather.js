...
function createCities() {
    ...
    for (var i = 0; i < cities.length; i++) {
       ...
       var van = city.city + ', ' + city.state;
       element.value = van;
       element.innerText = van
...
$(document).ready(function() {
    ...
    form.submit(function(e) {
        e.preventDefault();
        let city = $("#city").val();
        let formatedcity = city.replace(" ","");
            console.log(formatedcity);
        formatedcity = formatedcity.replace (" ","");
            console.log(formatedcity);
        formatedcity = formatedcity.replace (" ","");
            console.log(formatedcity);
        formatedcity = formatedcity.replace(",", "%2C%20");
            console.log(formatedcity);
        

        let url = "https://query.yahooapis.com/v1/public/yql?q=select%20item.condition%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22boston%2C%20ma%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys";
        url = url.replace  ("boston%2C%20", city)
        $.get(url, function(data) {
            let temp = data.query.results.channel.item.condition.temp
            $("#temp").html(temp)
            
            console.log(temp);
            let condition = data.query.results.channel.item.condition.text
            if (condition.toLowerCase().indexOf("sunny")!== -1) {
                $(".sunImage").addClass("show");
            } else {
                    $(".sunImage").removeClass("show");
            }
            $("#condition").html(condition);

            if (condition.toLowerCase().indexOf("cloudy")!== -1) {
                $(".cloudImage").addClass("show");
            } else {
                    $(".cloudImage").removeClass("show");
            }
            $("#condition").html(condition);
